<script setup>
  ...
  const firstRoom = ref('');
  data.userChoice.firstRoom = firstRoom;

  onBeforeMount(() => {
    if(route.query.customers && route.query.date){
      data.customers = route.query.customers;
      [data.userChoice.date.start, data.userChoice.date.end] = route.query.date.toLocaleString().split(",");

      data.userChoice.date.interval = ((new Date(data.userChoice.date.end).getTime() - new Date(data.userChoice.date.start).getTime()) / (1000 * 3600 * 24)).toFixed(0);

      getAvailable(data.userChoice.date.start, data.userChoice.date.end).then((response) => {
        data.items = response;
      });
    }
...
<template>
  <Header/>
  <div class="hrtext">
    <h2 class="mt-12 mb-12 ml-6">Chambre 1</h2>
    <div class="headerhr"></div>
  </div>
  <section class="flex justify-around">
    <el-radio-group v-for="(item, index) in data.items" :key="index" :index="item" v-model="firstRoom" size="large" fill="#00b561">
      <div class="flex justify-around gap-6  m-1">
  ...
  <div class="flex justify-center mt-12">
    <el-button v-if="firstRoom !== ''" size="large" color="#00B561" data-te-ripple-init data-te-ripple-color="light" @click="checkout">
      Checkout
    </el-button>
  </div>
</template>

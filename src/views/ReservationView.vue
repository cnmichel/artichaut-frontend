<script setup>
  import { getAvailable } from '@/services/api';
  ...
  onBeforeMount(() => {
    if(route.query.customers && route.query.date){
      data.customers = route.query.customers;
      [data.date.start, data.date.end] = route.query.date.toLocaleString().split(",");


      data.date.interval = (new Date(data.date.end).getTime() - new Date(data.date.start).getTime()) / (1000 * 3600 * 24);

      getAvailable(data.date.start, data.date.end).then((response) => {
        data.items = response;
...
<template>
  <h2>Chambre 1</h2>
  <el-radio-group v-for="(item, index) in data.items" :key="index" :index="item" v-model="Chambre1" size="large" fill="#00b561" >
    <div class="flex justify-around gap-6  m-12">
      <div class="relative mx-auto w-full">
        <div href="#" class="relative inline-block w-full">
          <div class="shadow p-4 rounded-lg bg-white">
            ...
            <div class="grid grid-cols-2 mt-8">
              <div class="flex items-center">
                <el-radio-button :label="'Réserver' + index" />
              </div>

              <div class="flex justify-end items-center">
                <p class="inline-block font-semibold text-tonic whitespace-nowrap leading-tight rounded-xl">
                  {{ (item?.price*data.customers)*data.date.interval }}€/sejour </p>
                <p> ({{item?.price*data.date.interval }}€/pers)</p>
              </div>
            </div>
          </div>
        </div>
  ...
  <el-button v-if="Chambre1 !== ''" :icon="Search" size="large" color="#00B561" data-te-ripple-init data-te-ripple-color="light" @click="handleSearch">
    <router-link to="/checkout" tag="button">Etape suivante</router-link>
  </el-button>
</template>
